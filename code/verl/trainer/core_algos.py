...
class AdvantageEstimator(str, Enum):
    ...
    GAE = "gae"
    GRPO = "grpo"
    GRPOREWEIGHT = "grporeweight"
    REINFORCE_PLUS_PLUS = "reinforce_plus_plus"
    REMAX = "remax"
    RLOO = "rloo"
    GRPOSUBTRACT = "grposubtract"
    GRPOMEAN = "grpomean"
    GRPOMIXPRO = "grpomixpro"
    GRPOMIXAIR = "grpomixair"
    GRPORANDPRO = "grporandpro"
    GPG = "gpg" 
    TREERPO="treerpo"
...
def compute_grpo_outcome_advantage(
    token_level_rewards: torch.Tensor, response_mask: torch.Tensor, index: torch.Tensor, eps: float = 1e-6) -> Tuple[torch.Tensor, torch.Tensor]:

    scores = token_level_rewards.sum(dim=-1) # scores: shapes (bs)
    ...
    return returns, returns, id2mean,  id2std, id2max

@torch.no_grad()
def compute_grpo_subtraction_outcome_advantage(
    token_level_rewards: torch.Tensor, response_mask: torch.Tensor, index: torch.Tensor, eps: float = 1e-6) -> Tuple[torch.Tensor, torch.Tensor]:
    ...
    returns = scores.unsqueeze(-1) * response_mask
    return returns, returns, id2mean, id2max

@torch.no_grad()
def compute_grpo_mean_outcome_advantage(
    token_level_rewards: torch.Tensor, response_mask: torch.Tensor, index: torch.Tensor, eps: float = 1e-6) -> Tuple[torch.Tensor, torch.Tensor]:

    scores = token_level_rewards.sum(dim=-1) # scores: shapes (bs)
    id2score = defaultdict(list)
    id2mean, id2std, id2max, id2count_ones= {}, {}, {} ,{}
    ...
    for idx in id2score:
        ...
        id2mean[idx] = torch.mean(torch.tensor(id2score[idx]))  
        id2max[idx] = torch.max(torch.tensor(id2score[idx]))
        id2count_ones[idx] = torch.sum(torch.tensor(id2score[idx])==1.0)

    for i in range(bsz):
        scores[i] = (scores[i] - id2mean[index[i]]) / (id2mean[index[i]] + eps)

    returns = scores.unsqueeze(-1) * response_mask
    return returns, returns, id2mean, id2max, id2count_ones

@torch.no_grad()
def compute_grpo_treerpo_outcome_advantage(
    token_level_rewards: torch.Tensor, response_mask: torch.Tensor, index: torch.Tensor, eps: float = 1e-6) -> Tuple[torch.Tensor, torch.Tensor]:
    ...
    for i in range(bsz):
        scores[i] = (scores[i] - id2mean[index[i]]) / (id2mean[index[i]]*(1-id2mean[index[i]]) + eps)
